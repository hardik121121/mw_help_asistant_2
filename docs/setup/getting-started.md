...
**📖 For comprehensive architecture documentation, see [Technical Architecture Guide](../technical/architecture.md)**
...
│                                                                      │
│  ┌────────────────┐  ┌────────────────┐  ┌────────────────┐       │
│  │   Pinecone     │  │   BM25 Index   │  │  Embeddings    │       │
│  │  (Cloud DB)    │  │   (Local)      │  │    (Local)     │       │
│  │                │  │                │  │                │       │
│  │ 2,106 vectors  │  │ 16,460 terms   │  │ Content maps   │       │
│  │ 3072-dim       │  │                │  │ Metadata maps  │       │
│  └────────────────┘  └────────────────┘  └────────────────┘       │
└─────────────────────────────────────────────────────────────────────┘
         ↑                          ↑                       ↑
         │                          │                       │
...
│                                                                      │
│  PDF (2,257 pages) → Docling → Hierarchical Chunker → Embeddings   │
│                                                                      │
│  Output: 2,106 chunks with full metadata + images                  │
└─────────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  User Query → Query Understanding Pipeline                          │
│                                                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ 1. Query Decomposer (LLM-based)                              │  │
│  │    - Breaks complex queries into sub-questions                │  │
│  │    - Uses Groq Llama 3.3 70B                                 │  │
│  │    - Example: "Create block + test it" → 2 sub-questions    │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                          ↓                                           │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ 2. Query Classifier (Rule-based)                             │  │
│  │    - Identifies question type (procedural, conceptual, etc.) │  │
...
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                      │
│  Output: QueryUnderstanding object with sub-questions + metadata    │
└─────────────────────────────────────────────────────────────────────┘
                                    ↓
┌─────────────────────────────────────────────────────────────────────┐
...
│                          ↓                                           │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ 3. Keyword Boosting (MS Teams Fix!)                          │  │
│  │    - Detect exact integration names                          │  │
│  │    - Apply 10x score boost                                   │  │
│  │    - Skip reranking if exact match found                     │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                          ↓                                           │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ 4. Semantic Reranking (if no exact match)                    │  │
│  │    - Cohere rerank-english-v3.0                              │  │
│  │    - Re-scores top 30 results                                │  │
│  │    - Keeps top 10 after reranking                            │  │
│  │    - Cost: ~$0.0005 per rerank                               │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
...
│  │    - Sort by: boosted flag → score → section → page         │  │
│  │    - Group by topic for readability                          │  │
│  │    - Keep top 20 chunks for LLM                              │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                      │
│  Output: OrganizedContext with 20 ranked chunks + metadata          │
└─────────────────────────────────────────────────────────────────────┘
                                    ↓
┌─────────────────────────────────────────────────────────────────────┐
...
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ 1. Format Context                                             │  │
│  │    - Convert 20 chunks to markdown format                    │  │
│  │    - Preserve ranking order                                  │  │
│  │    - Include section headings and page numbers               │  │
│  │    - Add note: "First sections are most relevant"            │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                          ↓                                           │
│  ┌──────────────────────────────────────────────────────────────┐  │
...
│  │    - Image referencing instructions                          │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                          ↓                                           │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ 4. Generate Answer                                            │  │
...
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                      │
│  Output: GeneratedAnswer with text + citations + images             │
└─────────────────────────────────────────────────────────────────────┘
                                    ↓
┌─────────────────────────────────────────────────────────────────────┐
│                     STAGE 4: VALIDATION                              │
...
│  │    - Does answer address all sub-questions?                  │  │
...
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                      │
│  Output: ValidationResult with scores + issues                      │
└─────────────────────────────────────────────────────────────────────┘
                                    ↓
...
┌─────────────┐
│   PDF File  │  157 MB, 2,257 pages
└──────┬──────┘
       │
       ↓ Docling Processing (~15-60 min)
┌──────────────────┐
│  Structured JSON │  43 MB with layout, tables, images
└──────┬───────────┘
       │
       ↓ Hierarchical Chunking (~1-2 min)
┌─────────────────────┐
│  2,106 Chunks       │  Each chunk includes:
│  + Full Metadata    │  - Content (500-2000 chars)
└──────┬──────────────┘  - Section hierarchy
       │                  - Images, tables
       │                  - 20+ metadata fields
       │
       ...
       ↓                  ↓                  ↓
┌────────────┐   ┌─────────────┐   ┌──────────────┐
│ Embeddings │   │ Pinecone DB │   │  BM25 Index  │
│ Generation │   │   Upload    │   │  Creation    │
└────────────┘   └─────────────┘   └──────────────┘
  (~5 min)          (~2 min)          (<1 min)
...
Query Understanding (~1-2s, FREE)
    ...
    ↓
    ├─ Sub-question 1 ────┐
    ├─ Sub-question 2 ────┤ Process in sequence
    └─ Sub-question 3 ────┤ (~3-4s each)
                          ↓
                    For Each Sub-Q:
                          │
                          ├─ Generate Embedding (~0.3s, $0.0001)
                          │       ↓
                          ├─ Hybrid Search (~0.5s, FREE)
                          │    ├─ Vector Search (Pinecone)
                          │    ├─ BM25 Search (Local)
                          │    └─ RRF Fusion
                          │       ↓
                          ├─ Keyword Boosting (~0.1s, FREE)
                          │       ↓
                          ├─ Reranking (~1-2s, $0.0005)
                          │    (Cohere API)
                          │       ↓
                          └─ Results (10 chunks)
                                  ↓
...
│  Rank-BM25                  Local keyword search                │
│  Pickle                     Local embeddings storage            │
└─────────────────────────────────────────────────────────────────┘
...
│  BeautifulSoup              HTML/text parsing                   │
...
app.py (Streamlit UI)
    ↓
    └─→ src.generation.end_to_end_pipeline
            ↓
            ├─→ src.query.query_understanding
            │       ├─→ src.query.query_decomposer (Groq LLM)
            │       ├─→ src.query.query_classifier (Rules)
            │       └─→ src.query.intent_analyzer
            │
            ├─→ src.retrieval.multi_step_retriever
            │       ├─→ src.database.embedding_generator (OpenAI)
            │       ├─→ src.retrieval.hybrid_search
            │       │       ├─→ Pinecone (Cloud API)
            │       │       └─→ Rank-BM25 (Local)
            │       ├─→ src.retrieval.reranker (Cohere)
            │       └─→ src.retrieval.context_organizer
            │
            ├─→ src.generation.answer_generator (Groq LLM)
            │
            └─→ src.generation.response_validator
...
Converts PDF → Structured chunks ready for search
...
### 📁 `/src/query` - Question Understanding
...
### 📁 `/src/generation` - Answer Creation
...
| `end_to_end_pipeline.py` | Complete pipeline (question → answer) |

### 📁 `/src/evaluation` - Quality Testing
...
- Retrieve for both → Combine → Generate unified answer
...
**Why These Values**:
- 1000 tokens ≈ 750 words (enough for complete thoughts)
...
**Cost**: FREE (Groq free tier: 100K tokens/day ≈ 14 queries)
...
|--------|-------|--------|--------|
| **Search Quality** | | | |
| Precision@10 | 0.567 | >0.70 | ⚠️ Needs improvement |
| Recall@10 | 0.551 | >0.60 | ⚠️ Needs improvement |
| Diversity | 1.000 | >0.70 | ✅ Perfect |
| **Answer Quality** | | | |
| Overall Score | 0.916 | >0.75 | ✅ Excellent |
| Completeness | 1.000 | >0.80 | ✅ Perfect |
| Word Count | 484 words | 400-600 | ✅ Ideal |
| **Performance** | | | |
| Avg Time per Query | 27.4s | <15s | ⚠️ Needs optimization |
...
|---------|------|---------|
| OpenAI Embeddings | $0.0005 | 3 sub-questions × 1 embedding each |
| Cohere Reranking | $0.0015 | 3 sub-questions × 1 reranking each |
...
See `CLAUDE.md` → Upgrade Recommendations section.
...
**A**: Partially:
- ✅ **BM25 search**: Fully offline (local index)
- ❌ **Vector search**: Requires Pinecone API (cloud)
- ❌ **Embeddings**: Requires OpenAI API
- ❌ **Reranking**: Requires Cohere API
- ❌ **LLM generation**: Requires Groq API
...
```bash
python -m src.module.name  # ✅ Correct
python src/module/name.py  # ❌ Wrong
```

### Error: "Rate limit reached for model llama-3.3-70b-versatile"

**Solution**: Groq free tier limit reached (100K tokens/day ≈ 14 queries)
...
1. Check `CLAUDE.md` → Common Patterns section
...
1. Check `CLAUDE.md` → Upgrade Recommendations
